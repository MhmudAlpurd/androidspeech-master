...
public class MainActivity extends AppCompatActivity implements
    ...
    private void initialize() {
        ...
        mtxtinftime = findViewById(R.id.txt_inf_time);
    ...
    public void onSTTResult(@Nullable STTResult result) {
        if (result != null) {
            ...
            String message = String.format("Success-> Text: %s , Conf.: %s", result.mTranscription, result.mConfidence);
            mLogText.append(message + "\n");
            endTime= new Date().getTime();
            inferenceTime = endTime - startTime ;

            String finalResult_str = result.mTranscription;
            String[] splited = finalResult_str.split("\\s+");
            long totalCharacters = finalResult_str.chars().count();
            int num_of_words = splited.length;
            int inf_per_word = (int) (inferenceTime / num_of_words);
            int inf_per_char = (int) (inferenceTime / totalCharacters);

            String inference_info = String.format("Inference Time: %s , Inf/Word: %s, Inf/Char: %s", inferenceTime, inf_per_word, inf_per_char);
            mtxtinftime.setText(inference_info);
        }
        mStartButton.setEnabled(true);
    }
